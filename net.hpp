...
ENetPacket* enet_packet_create(	const void * data, uint32_t 	dataLength, enet_uint32 flags, const PacketType packetType);
enet_uint8 * enet_packet_data(ENetPacket * packet);
PacketType enet_packet_type(ENetPacket * packet);
uint32_t enet_packet_size(ENetPacket * packet);


/// creates an enet packet with a packetType added in the beginning
ENetPacket* enet_packet_create(	const void * 	data, uint32_t 	dataLength, enet_uint32 	flags, PacketType packetType) {
